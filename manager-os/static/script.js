document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateConnectionStatus(connected) {
        if (connected) {
            connectBtn.className = 'w-full bg-green-600 hover:bg-green-700 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all';
            startKioskBtn.classList.remove('hidden');
            startKioskBtn.classList.add('flex');
        } else {
            connectBtn.className = 'w-full bg-red-600 hover:bg-red-700 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all';
    ...
    if (wifiForm) {
        wifiForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (result.success) {
                    statusMessage.textContent = 'WiFi configured successfully! Checking connection...';
                    ...
                    const quickCheck = setInterval(async () => {
                        ...
                        try {
                            ...
                            if (data.connected) {
                                ...
                                statusMessage.textContent = 'Connected! You can now start the kiosk.';
                                statusMessage.className = 'text-sm text-center text-green-400';
                                // Resume normal checking interval
                                wifiCheckInterval = setInterval(checkWiFiStatus, 3000);
                            } else if (checkAttempts >= maxAttempts) {
                                clearInterval(quickCheck);
                                statusMessage.textContent = 'Connection timeout. Please check credentials and try again.';
